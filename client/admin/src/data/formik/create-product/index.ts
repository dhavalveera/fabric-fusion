...
export const createProductValidationSchemas: Yup.ObjectSchema<Partial<CreateProductFormValues>>[] = [
  ...
  Yup.object<Partial<CreateProductFormValues>>({
    careInstruction: Yup.object({
      bleachingInstructions: Yup.string().max(255, "Max 255 characters are allowed").notRequired(),
      dryCleaningInstructions: Yup.string().max(255, "Max 255 characters are allowed").notRequired(),
      dryingInstructions: Yup.string().max(255, "Max 255 characters are allowed").notRequired(),
      ironingInstructions: Yup.string().max(255, "Max 255 characters are allowed").notRequired(),
      storageInstructions: Yup.string().max(255, "Max 255 characters are allowed").notRequired(),
      washingInstructions: Yup.string().max(255, "Max 255 characters are allowed").notRequired(),
    }),
